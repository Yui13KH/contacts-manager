...
namespace Contacts
{
    public partial class frmAddEditContact : Form
    {
        ...
        private void _FillCountriesInComoboBox()
        {
            ...
            cbCountry.DisplayMember = "CountryName";  // what the user sees
            cbCountry.ValueMember = "CountryID";      // the hidden value we can use
        }
        private void _LoadData()
        {
            ...
            llRemoveImage.Visible = !string.IsNullOrEmpty(_Contact.ImagePath);

            // Safely select country
            var country = clsCountry.Find(_Contact.CountryID);
            if (country != null)
            {
                int index = cbCountry.FindString(country.CountryName);
                if (index >= 0)
                    cbCountry.SelectedIndex = index;
            }
            else
            {
                // Optional: warn that country not found
                MessageBox.Show("Warning: Country not found for this contact.");
                cbCountry.SelectedIndex = 0;
            }
        }
